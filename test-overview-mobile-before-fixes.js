...
async function testOverviewMobileBeforeFixes() {
  ...
  console.log('🧪 Testing Overview Mobile Responsiveness (Before Fixes)');
  ...
  for (const viewport of viewports) {
    ...
    try {
      // Navigate to overview page (root)
      await page.goto('http://localhost:3000/', { waitUntil: 'networkidle' });
      await page.waitForTimeout(5000); // Wait longer for charts and data to load
      ...
      const screenshotPath = `overview-mobile-before-fixes-${viewport.name.toLowerCase().replace(/\s+/g, '-')}.jpg`;
...
testOverviewMobileBeforeFixes().catch(console.error);