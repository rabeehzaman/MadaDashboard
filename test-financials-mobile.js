...
/**
 * Mobile responsiveness test for financials tab
 * Usage: node test-financials-mobile.js
 */
...
async function testViewportResponsiveness(page, viewportName, viewport) {
  ...
  try {
    const screenshotResult = await captureScreenshotSafe(page, {
      ...
      quality: 85,
      format: 'jpeg',
      path: `financials-mobile-test-${viewportName.toLowerCase().replace(/\s+/g, '-')}.jpg`,
      fullPage: true
    });
  ...
  const elementsToCheck = [
    ...
    { selector: '.grid', name: 'Financial cards grid' },
...
async function testFinancialsMobile() {
  console.log('📱 Starting Mobile Responsiveness Test for Financials Tab\n');
  ...
  try {
    ...
    for (const port of ports) {
      const url = `http://localhost:${port}/financials`;
      try {
    ...
    if (!connected) {
      throw new Error('Could not connect to financials page. Make sure your dashboard is running.');
    }
    ...
    console.log('\n🔍 FINANCIALS MOBILE RESPONSIVENESS TEST REPORT');
    console.log('=====================================');
    console.log(`📊 Summary:`);
    ...
    console.log(`\n🔗 Tested URL: ${financialsUrl}`);
    console.log('\n✅ Financials mobile responsiveness test completed!');

  } catch (error) {
    console.error('❌ Test failed:', error.message);
    console.log('\n🔧 Troubleshooting:');
    console.log('   - Make sure your dashboard is running (npm run dev)');
    console.log('   - Check if /financials route is accessible');
    process.exit(1);
  } finally {
    await browser.close();
  }
...
// Run the test
testFinancialsMobile().catch(console.error);